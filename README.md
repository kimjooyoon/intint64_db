...
- 이 시스템은 race condition 을 피하려고 하지 않는다.
- race condition 이 발생할 수 없는 구조로 설계되었다.

#### mmap 을 사용하는 이유

- **고정 크기**: 파일 크기가 한번 정해지면 변하지 않으므로, mmap 에 필요한 고정된 주소 공간이 보장된다. 매핑 후 파일 확장·축소가 없어 재매핑 이슈가 없다.
- **단일 수정 스레드**: 쓰기는 액터 한 스레드만 수행한다. mmap 영역에 대한 동시 쓰기가 없어, 별도 lock 없이 메모리 접근만으로 읽기·쓰기가 가능하다.
- **주기 저장**: 디스크 반영은 주기적으로 msync(또는 동등한 flush)로 수행한다. “시간 주기적으로 저장”이라는 설계와 동일한 의미로 적용 가능하다.
- **중단 허용**: msync 전에 프로세스가 중단되면 그 구간만 유실된다. “중단 허용”을 전제로 한 특수 목적이므로, 이 트레이드오프가 수용 가능하다.
- **성능**: Read/Write 가 syscall(ReadAt/WriteAt) 대신 메모리 접근으로 바뀌어, 랜덤 접근이 빈번한 패턴에서 지연이 줄고 처리량이 늘어난다.
...
- 0.7.n.m :
  - 시간 퀀타이즈 기준n을 m부터 시작으로 적용
  - bug) 0.6.n 커맨드 사전 적용된 후 0.7.n.m 사용해야함
...
- 2.1.id1.id2 : 
  - id1 부터 id2 까지의 값을 큰 값을 순서로 정렬하여 저장하고 큰값부터 작은 값까지를 출력
